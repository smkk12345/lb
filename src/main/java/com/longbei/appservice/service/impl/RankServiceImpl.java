...
//    }
     ...
     * @return
     */
    public  int getSureRemoveRankMemberCount(Long rankId) {
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("rankId",rankId);
...
//            String remark = Constant.MSG_QUITRANK_MODEL.replace("n", rank.getRanktitle());
            if("0".equals(rank.getSourcetype()) || "1".equals(rank.getSourcetype())){
            	//mtype 0 系统消息(msgtype  18:升龙级   19：十全十美升级   20:榜关注开榜通知    21：榜关注结榜通知
								...
								//32：创建的龙榜/教室/圈子被选中推荐 53：被授予龙杯名人认证 54: 教室成员退出教室 55：转让群主权限
                                // 56:被授予龙杯Star认证 57:教室关闭 58:教室课程更新 59:删除教室成员进步 60:教室发布新公告
                                // 61:教室疑答回复 62:邀请注册进步币奖励 63:教室收益提现到钱包 64:教室提现到银行卡，支付宝
                                // 65:老师认证审核结果
								//40：订单已取消 41 榜中进步下榜
...
//    @Transactional(propagation= Propagation.SUPPORTS)
    ...
    public BaseResp<Object> acceptAward(Long userId, Long rankId) {
        ...
        try{
            ...
            if(award == null){
                logger.error("query award null awardId:{}",rankMember.getRankAward().getAwardid());
    ...
    public BaseResp<Object> selectWinningRankAward(Long userid) {
        BaseResp<Object> baseResp = new BaseResp<Object>();
        try{
            List<Map<String,Object>> resultList = rankCache.selectRankWinningAwardList();
            if(!Constant.VISITOR_UID.equals(userid)){
                for (Map<String,Object> map : resultList){
                    map.put("nickname",this.userRelationService.
                            getUserRemark(userid,Long.parseLong(map.get("awarduserid").toString()),true));
                }
            }
            baseResp.setData(resultList);
            return baseResp.initCodeAndDesp(Constant.STATUS_SYS_00,Constant.RTNINFO_SYS_00);
...
//            if (Constant.VISITOR_UID.equals(userId)){
                for (Map<String,Object> map : list){
                    ...
                    for (Map<String,Object> m : ls){
                        m.put("usernickname",userRelationService.getUserRemark(userId,
...
//                String remark = Constant.MSG_QUITRANK_IMP_MODEL.replace("n", rank.getRanktitle());
                    ...
                    //						29：被PC选为热门话题    30：被选为达人   31：微进步被推荐
                    //						32：创建的龙榜/教室/圈子被选中推荐 53：被授予龙杯名人认证 54: 教室成员退出教室 55：转让群主权限
                    //                      56:被授予龙杯Star认证 57:教室关闭 58:教室课程更新 59:删除教室成员进步 60:教室发布新公告 61:教室疑答回复
                    //                      62:邀请注册进步币奖励 63:教室收益提现到钱包 64:教室提现到银行卡，支付宝
                    //                      65:老师认证审核结果
                    //						40：订单已取消 41 榜中进步下榜
